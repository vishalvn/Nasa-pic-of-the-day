...
import com.vv.nasapod.databinding.ScreenFirstBinding
import com.vv.nasapod.getAppComponent
import com.vv.nasapod.util.TYPE_IMAGE
import com.vv.nasapod.util.TYPE_VIDEO
import com.vv.nasapod.util.getYoutubeVideoIdFromUrl
import com.vv.nasapod.util.retrieveVideoFrame
import com.vv.nasapod.network.NetworkState.*
...
class NasaPODScreen : Fragment(R.layout.screen_first) {

    @Inject
    lateinit var nasaAPODViewModelFactory: NasaPODViewModelFactory
    @Inject
    lateinit var picasso: Picasso
    private lateinit var binding: ScreenFirstBinding
    private lateinit var videoLoadDisposable : Disposable
    private val nasaPODViewModel: NasaPODViewModel by navGraphViewModels(R.id.nav_main) { nasaAPODViewModelFactory }
    ...
    private fun subscribeMediaType() {
        nasaPODViewModel.mediaType.observe(this.viewLifecycleOwner, Observer {
            if (it != null) {
                when (it) {
                    TYPE_IMAGE -> {
    ...
    private fun subscribeApodData() {
        nasaPODViewModel.apodData.observe(this.viewLifecycleOwner, Observer {
            when (it) {
                //is Loading -> {
    ...
    private fun subscribeHdUrl() {
        nasaPODViewModel.hdUrl.observe(this.viewLifecycleOwner, Observer {
    ...
    private fun makeSnackBar() {
        snackbar = Snackbar.make(
            ...
            .setAction(getString(R.string.action_retry)) {
                nasaPODViewModel.getApodForDate()
            }
    ...
    private fun showDatePicker() {
        ...
        val datePicker = DatePickerDialog(
            ...
            dateSetListener,
            nasaPODViewModel.getDate().get(Calendar.YEAR),
            nasaPODViewModel.getDate().get(Calendar.MONTH),
            nasaPODViewModel.getDate().get(Calendar.DAY_OF_MONTH)
        )
