package com.vv.nasapod.ui.npod

import androidx.lifecycle.LiveData
import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel
import com.vv.nasapod.data.model.Npod
import com.vv.nasapod.network.NetworkState
import com.vv.nasapod.network.NetworkState.*
import com.vv.nasapod.util.TYPE_IMAGE
import com.vv.nasapod.util.TYPE_VIDEO
import io.reactivex.android.schedulers.AndroidSchedulers
import io.reactivex.disposables.CompositeDisposable
import io.reactivex.schedulers.Schedulers
import java.io.IOException
import java.text.DecimalFormat
import java.util.*
import javax.inject.Inject

class NasaPODViewModel  @Inject constructor(
    private val nasaAPODRepository: NasaPODRepository
) : ViewModel() {

    private val selectedDateCalenar = Calendar.getInstance(TimeZone.getTimeZone("UTC"))
    private val currentDateCalendar = Calendar.getInstance(TimeZone.getTimeZone("UTC"))
    private val decimalFormat: DecimalFormat by lazy { DecimalFormat("00") }
    private val _hdUrl : MutableLiveData<String> = MutableLiveData()
    val hdUrl : LiveData<String> get() = _hdUrl
    private val _mediaType : MutableLiveData<String> = MutableLiveData()
    val mediaType : MutableLiveData<String> get() = _mediaType
    private val defaultDate: String =
        "${decimalFormat.format(selectedDateCalenar.get(Calendar.YEAR))}-${decimalFormat.format(
            selectedDateCalenar.get(Calendar.MONTH) + 1)}-${decimalFormat.format(
            selectedDateCalenar.get(Calendar.DAY_OF_MONTH)
        )}"
    private val compositeDisposable: CompositeDisposable by lazy { CompositeDisposable() }
    private val _selectedDate: MutableLiveData<String> =
        MutableLiveData(defaultDate) // date in yyyy-mm-dd
    private val _apodData: MutableLiveData<NetworkState<Npod>> = MutableLiveData()
    val apodData: LiveData<NetworkState<Npod>> get() = _apodData

    init {
        getDefaultApod()
    }

    private fun getDefaultApod() {
        _apodData.value = NetworkState.Loading
        compositeDisposable.add(
            nasaAPODRepository.getDefaultNpod()
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({
                    if (it != null) {
                        _apodData.value = Success(it)
                        if (it.mediaType == TYPE_IMAGE){
                            _mediaType.value = TYPE_IMAGE
                            _hdUrl.value = it.hdurl
                        }
                        else {
                            _mediaType.value = TYPE_VIDEO
                            _hdUrl.value = it.url
                        }
                    } else {
                        _apodData.value = Error("Error fetching data!")
                    }
                }, {
                    if (it is IOException) {
                        _apodData.value = NetworkError
                    } else {
                        _apodData.value = Error("${it.message}")
                    }
                })
        )
    }


    fun getApodForDate() {
        _apodData.value = Loading
        compositeDisposable.add(
            nasaAPODRepository.getNpod(_selectedDate.value)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({
                    if (it != null) {
                        _apodData.value = Success(it)
                        if (it.mediaType == TYPE_IMAGE){
                            _mediaType.value = TYPE_IMAGE
                            _hdUrl.value = it.hdurl
                        }
                        else {
                            _mediaType.value = TYPE_VIDEO
                            _hdUrl.value = it.url
                        }
                    } else {
                        _apodData.value = Error("Error fetching data!")
                    }
                }, {
                    if (it is IOException) {
                        _apodData.value = NetworkError
                    } else {
                        _apodData.value = Error("${it.message}")
                    }
                })
        )
    }

    fun setDate(year : Int, month : Int, day : Int) {
        selectedDateCalenar.set(Calendar.YEAR,year)
        selectedDateCalenar.set(Calendar.MONTH,month)
        selectedDateCalenar.set(Calendar.DAY_OF_MONTH,day)
        if(selectedDateCalenar.time == currentDateCalendar.time) {
            getDefaultApod()
        }else {
            this._selectedDate.value = "${decimalFormat.format(year)}-${decimalFormat.format(
                month + 1)}-${decimalFormat.format(day)}"
            getApodForDate()
        }
    }

    fun getDate() : Calendar = selectedDateCalenar

    fun reloadVideo() {
        this._hdUrl.value = _hdUrl.value
    }

    override fun onCleared() {
        super.onCleared()
        compositeDisposable.dispose()
    }

}